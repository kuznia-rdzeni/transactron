...
from transactron.core.context import TransactronContextComponent
...
class TestGenerateVerilog:
    def test_gen_info_evlog(self, tmp_path):
        from transactron.utils.gen import GenerationInfo

        with DependencyContext(DependencyManager()):
            DependencyContext.get().add_dependency(EvLogEnabledKey(), True)
            circ = GenTestCircuit()
            top = TransactronContextComponent(circ, dependency_manager=DependencyContext.get())

            verilog_text, gen_info = generate_verilog(top)

        evlog = gen_info.evlog
        assert len(evlog.schema.sites) == 1
        site = evlog.schema.sites[0]
        assert site.event_name == "test.gen_event"
        assert site.source_name == "test.gen"
        assert site.fields == [EventFieldSchema(name="value", width=8)]

        assert evlog.triggers_location is not None
        assert len(evlog.site_locations) == 1
        assert len(evlog.site_locations[0].fields) == 1

        # The packed trigger vector must be locatable in the generated Verilog.
        assert "evlog_triggers" in verilog_text

        # GenerationInfo JSON roundtrip preserves the event log information.
        path = str(tmp_path / "gen_info.json")
        gen_info.encode(path)
        decoded = GenerationInfo.decode(path)
        assert decoded.evlog.schema.sites[0].event_name == "test.gen_event"
        assert decoded.evlog.site_locations[0].trigger == evlog.site_locations[0].trigger
        assert decoded.evlog.triggers_location == evlog.triggers_location
