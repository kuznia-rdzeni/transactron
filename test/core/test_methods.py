...
class TestSelfCallNonexclusive(TestCaseWithSimulator):
    def test_self_call_nonexclusive(self):
        class SelfCallNonexclusive(Elaboratable):
            def __init__(self):
                self.meth = Method(i=[("count", 4)])
                self.count = Signal(4)
            ...
            def elaborate(self, platform):
                ...
                @def_method(m, self.meth, nonexclusive=True, combiner=combiner)
                def _(count):
                    m.d.av_comb += self.count.eq(count)
        ...
        m = SelfCallNonexclusive()
        dut = SimpleTestCircuit(m)

        async def process(sim):
            await sim.tick()
            expected_count = 3  # the method calls itself twice
            assert sim.get(m.count) == expected_count
